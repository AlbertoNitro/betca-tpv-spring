...
public interface ArticleRepository extends MongoRepository<Article, String> {
    ...
    @Query(value = "{$or:[" + "{reference : {$in : [null, ''] }}," + "{description : {$in : [null, ''] }},"
            + "{retailPrice : {$in : [null, 0] }}," + "{stock : null }," + "{provider : null }"
    ...
    @Query(value = "{$and:[" + "?#{ [0] == null ? { $where : 'true'} : { 'reference' : {$regex:[0], $options: 'i'} } },"
