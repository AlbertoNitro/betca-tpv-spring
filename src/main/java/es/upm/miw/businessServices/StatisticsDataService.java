...
public class StatisticsDataService {
    ...
    public List<HistoricalProductOutPutDto> GetHistoricalData(Date initDate, Date endDate) {
        ...
        if (endCalendar.get(Calendar.YEAR) > initCalendar.get(Calendar.YEAR)) {
            endMonth = endCalendar.get(Calendar.MONDAY) + 12 * (endCalendar.get(Calendar.YEAR) - initCalendar.get(Calendar.YEAR));
        }
        ...
        for (Map.Entry<String, HashMap<Integer, Integer>> articleData : resultAux.entrySet()) {
            ...
            HistoricalProductOutPutDto item = new HistoricalProductOutPutDto(numArticlePerMonthCollection, articleData.getKey());
            result.add(item);
        }
        ...
        for (Ticket ticket : tickectsCollection) {
            ...
            for (Shopping shopping : ticket.getShoppingList()) {
                ...
                if (!mapAux.containsKey(articleAux.getReference())) {
                    ...
                    mapAux.put(articleAux.getReference(), ((int) mapAux.get(articleAux.getReference())) + shopping.getAmount());
    ...
    public List<IncomeComparision> GetIncomeComparisionData(Date initDate, Date endDate) {
        ...
        for (Ticket ticket : tickectsCollection) {
            ...
            for (Shopping shopping : ticket.getShoppingList()) {
                ...
                BigDecimal income = shopping.getDiscount().intValue() > 0 ? productPrice
                        .subtract(productPrice.multiply(shopping.getDiscount().divide(new BigDecimal(100)))) : productPrice;

                if (!mapAuxProductPrice.containsKey(articleAux.getReference()) && !mapAuxIncome.containsKey(articleAux.getReference())) {
                    ...
                    mapAuxIncome.put(articleAux.getReference(), mapAuxIncome.get(articleAux.getReference()) + income.floatValue());
        ...
        for (Map.Entry<String, Float> incomeItem : mapAuxIncome.entrySet()) {
            result.add(new IncomeComparision(mapAuxProductPrice.get(incomeItem.getKey()), incomeItem.getValue(), incomeItem.getKey()));
        }
